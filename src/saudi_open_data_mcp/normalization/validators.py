...
from .field_mapping import FieldMappingResult, MappingBodyKind
...
def validate_field_mapping(mapping_result: FieldMappingResult) -> FieldMappingValidationResult:
    ...
    if mapping_result.body_kind is MappingBodyKind.JSON:
        _validate_json_mapping(mapping_result)
...
def _validate_json_mapping(mapping_result: FieldMappingResult) -> None:
    """Validate a JSON mapping that should be record-derivable."""
    ...
    if not mapping_result.can_derive_records:
        raise ValueError("json mapping must be marked as record-derivable")

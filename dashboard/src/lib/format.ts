...
export function formatAge(seconds: number | null): string {
  ...
  if (days > 0) return `${formatAgePart(days, "يوم")} ${formatAgePart(hours, "ساعة")}`;
  const minutes = Math.floor((seconds % 3600) / 60);
  if (hours > 0) {
    return `${formatAgePart(hours, "ساعة")} ${formatAgePart(minutes, "دقيقة")}`;
  }
  return formatAgePart(minutes, "دقيقة");
}

function formatAgePart(value: number, unitLabel: string): string {
  return `${NUMBER_FORMATTER.format(value)} ${unitLabel}`;
}
